...
export const findType = () => {
  ...
  switch (typeof thing) {
    ...
    default:
      output = "I don't know what this thing is";
  }
  return output;
};
...
export const getIsValidOnNameTag = () => {
  if (nameTagOption[0] == nameTagOption[0].toUpperCase()) {
...
export const convertPascalCaseToSnakeCase = () => {
  let outputString = pascalCaseVariableName[0].toLowerCase();
  for (let i = 1; i < pascalCaseVariableName.length; i++) {
    if (pascalCaseVariableName[i] == pascalCaseVariableName[i].toUpperCase()) {
      outputString =
        outputString + "_" + pascalCaseVariableName[i].toLowerCase();
    } else outputString = outputString + pascalCaseVariableName[i];
  }
  return outputString;
};
