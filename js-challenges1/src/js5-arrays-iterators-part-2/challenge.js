...
export const totalScoresArr = (scoresArr) => {
  let output = scoresArr.reduce(thisFunc);
  function thisFunc(total, num) {
    return total + num;
  }
  return output;
...
export const sortCharactersAlphabetically = (charcterArr) => {
  return charcterArr.join("").toLowerCase().split("").sort();
};
...
export const checkItemInstock = (toCheck) => {
  ...
  for (let i = 0; i < stockList.length; i++) {
    if (toCheck == stockList[i]) {
      return stockList[i] + " is instock, it is on aisle " + i + ".";
    } else outputString = "Sorry " + toCheck + " is not instock.";
  }
  return outputString;
};
...
export const checkPrimaryColours = (coloursArr) =>
  coloursArr.every((color) => color.match("red|yellow|blue")); // sara's solution
...
export const checkStringPalindrome = (stringOne) =>
  stringOne == stringOne.split("").reverse().join("");
// performance isnt good with this because it loops 3 times
...
export const totalNestedScoresArr = (scoresArr) => {
  ...
  for (let i = 0; i < scoresArr.length; i++) {
    outputArray.push(scoresArr[i].reduce(thisFunc));
  }
  function thisFunc(total, num) {
    return total + num;
  }
