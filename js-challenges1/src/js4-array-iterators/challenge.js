...
export const removeFalseValues = (booleanArr) => {
  let outputArray = booleanArr.filter((input) => input);
  return outputArray;
};
...
export const createPercentageList = (numbersArr) => {
  let outputArray = numbersArr.map((number) => {
    return number * 100 + "%";
  });
  return outputArray;
};
...
export const createListOfPoessessions = (possessionsArr, name) => {
  ...
  for (let i = 0; i < possessionsArr.length; i++) {
    outputArray.push(`${name} ${possessionsArr[i]}`);
  }
  return outputArray;
};
 ...
 * The function needs o remove any book titles that do not include the search term.
 *
...
export const formatString = (string) => {
  let checkString = string.replace(/[^A-Za-z]/g, "");
  let outputArray = [];
  for (let i = 0; i < checkString.length; i++) {
    if (i % 2 == 0) {
      outputArray.push(checkString[i].toUpperCase());
    } else outputArray.push(checkString[i].toLowerCase());
  }
  return outputArray;
};
...
export const fizzBuzz = (mixedArray) => {
  ...
  for (let i = 0; i < mixedArray.length; i++) {
    if (Number(mixedArray[i]) != NaN && Number(mixedArray[i]) > 0) {
      checkArray.push(Number(mixedArray[i]));
    }
  }
  for (let j = 0; j < checkArray.length; j++) {
    if (checkArray[j] % 15 == 0) {
      outputArray.push("FizzBuzz");
    } else if (checkArray[j] % 5 == 0) {
      outputArray.push("Buzz");
    } else if (checkArray[j] % 3 == 0) {
      outputArray.push("Fizz");
    } else outputArray.push(checkArray[j].toString());
  }
