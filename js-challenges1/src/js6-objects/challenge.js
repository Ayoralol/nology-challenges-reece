...
export const splitFullNameToFirstAndLast = (customer) => {
  let fullName = customer.fullName.split(" ");
...
export const mergeFurniture = (furnitureLocationData, furnitureProductData) => {
  // let fullData = {};
  // for (let key in furnitureLocationData) {
  //   fullData[key] = furnitureLocationData[loc];
  // }
  // for (let key in furnitureProductData) {
  //   fullData[key] = furnitureProductData[key];
  // }
  // return fullData;
  const newObject = { ...furnitureLocationData, ...furnitureProductData };
  return newObject;
};
