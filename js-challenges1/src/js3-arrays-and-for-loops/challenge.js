...
export const getFirstAndLastItems = (itemsArr) => {
  let newArray = [itemsArr[0], itemsArr[itemsArr.length - 1]];
  return newArray;
};
...
export const generateAverage = (numberArr) => {
  ...
  let average = Math.round(count / numberArr.length);
  return average;
};
...
export const reverseOrder = (toReverseArr) => {
  ...
  for (let i = toReverseArr.length - 1; i >= 0; i--) {
    outputArray.push(toReverseArr[i]);
  }
  return outputArray;
};
...
export const generateHighscores = (playersArr, scoresArr) => {
  ...
  for (let i = 0; i < playersArr.length; i++) {
    outputArray.push(
      "P:" + (i + 1) + " " + playersArr[i] + " scored " + scoresArr[i]
    );
  }
  return outputArray;
};
...
export const encryptString = (toEncrypt) => {
  ...
  for (let i = 0; i < toEncrypt.length; i += 3) {
    out1 += toEncrypt[i];
    if (typeof toEncrypt[i + 1] != "undefined") {
      out2 += toEncrypt[i + 1];
    }
    if (typeof toEncrypt[i + 2] != "undefined") {
      out3 += toEncrypt[i + 2];
    }
  }

  let outputString = out1 + out2 + out3;
  return outputString;
};
